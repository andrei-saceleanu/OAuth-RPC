...
if [[ $1 =~ $numberPattern ]]; then
    avail=$(cat "tests/test$1/readme"|grep -oP "\d+")
fi
SERVER_NAME="server"
SERVER_PARAMS="tests/test$1/userIDs.db tests/test$1/resources.db tests/test$1/approvals.db $avail"
CLIENT_NAME="client"
...
if [[ $1 =~ $numberPattern ]]; then
    ...
    for i in {1..7}
    do
        avail=$(cat "tests/test$i/readme"|grep -oP "\d+")
        SERVER_PARAMS="tests/test$1/userIDs.db tests/test$1/resources.db tests/test$1/approvals.db $avail"
